...
server = function(input, output, session) {
  ...
  output$ANOVA_DIC_TABLE <- renderPrint({
    ...
    dic_easy <- ea1(dados, design =1, alpha = input$sigF_easy_dic / 100, list = lista())
    dic_easy
  })
  

  #######################################################
  # Tabela ANOVA DBC  EASYANOVA                         #
  #######################################################
  output$ANOVA_DBC_TABLE <- renderPrint({
    cat("Entrou na DBC Table")
    dataset=whichdataset()
    respo<-dataset[input$resp]
    respos <- unlist(respo)
    trata<-dataset[input$trat]
    ...
    cat(paste("Teste a: ", as.character(input$sigF_easy_dbc), "% de significância"))
    cat("\n\n")
    if (input$list_easy == "Uma"){
      dados <- base_individual
      resposta1=names(respo)
    ...
    dic_easy <- ea1(dados, alpha = input$sigF_easy_dbc / 100, list = lista())
    dic_easy
  })
  ...
  if(input$metodologia == 'expDes'){
    ...
    cat("Apos else")
    if (input$metodologia == 'easyanova' & input$deli_easyanova == 'dbc_easyanova'){
        tabsetPanel(
            tabPanel("ANOVA - DBC",       verbatimTextOutput("ANOVA_DBC_TABLE")),
      )
    }
  }
  })
